...
SignalProtocolStore.prototype = {
  ...
  archiveSiblingSessions(identifier) {
    ...
    return this.getDeviceIds(address.getName()).then(deviceIds => Promise.all(
  ...
  groupsCreateNewGroup(numbers, groupId) {
    ...
    return new Promise(function (resolve) {
      ...
      for (i of finalNumbers) groupObject.numberRegistrationIds[finalNumbers[i]] = {}

      this.putGroup(groupId, groupObject).then(() => ({ id: groupId, numbers: finalNumbers }))
    })
  },
  ...
  groupsAddNumbers(groupId, numbers) {
    this.getGroup(groupId).then(function (group) {
      ...
      for (i of numbers) {
        const number = numbers[i]
        if (!isNumberSane(number)) throw new Error('Invalid number in set to add to group')
  ...
  groupsUpdateNumbers(groupId, numbers) {
    this.getGroup(groupId).then(function (group) {
      if (group === undefined) throw new Error('Tried to update numbers for unknown group')

      if (numbers.filter(isNumberSane).length < numbers.length) throw new Error('Invalid number in new group members')
