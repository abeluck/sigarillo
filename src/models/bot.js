...
const Bot = {
  ...
  async findBotByToken(db, token) {
    const bot = await db('bots').where({ token }).first()
    return bot
  },
  async findBotForUser(db, userId, id) {
    const bot = await db('bots').where({ id, user_id: userId }).first()
    return bot
  },
  async findBotById(db, id) {
    const bot = await db('bots').where('id', id).first()
    return bot
  },
  async findBotByNumber(db, number) {
    const bot = await db('bots').where('number', number).first()
    return bot
  },
  ...
  async cycleToken(db, botId) {
    const newToken = uuid()
    await db('bots').where({ id: botId }).update({ token: newToken })
    return this.findBotById(db, botId)
  },
  async deleteBot(db, botId) {
    await db('bots').where({ id: botId }).del()
  },
}
