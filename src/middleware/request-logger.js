...
function requestLogger(app, logger) {
  ...
  return async (ctx, next) => {
    ...
    print({
      direction: '<--', directionColor: 'gray', method: ctx.method, path: ctx.originalUrl,
    })
...
const devPretty = winston.format((info, opts) => {
  const fields = []
  if (info.direction) fields.push(`${chalk.keyword(info.directionColor)(info.direction)}`)
...
export {
  requestLogger, devPretty, fileLogFilter,
}
